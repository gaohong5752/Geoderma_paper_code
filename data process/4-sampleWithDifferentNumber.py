# -*- coding: utf-8 -*-
"""
Created on Tue Jun 18 16:42:54 2019
...
pSNA_tif_n3=[i for i in pSNA_tif_Fn if factor in i and "NS3" in i ]
#pSNA_tif_n3=pSNA_tif_n3[0:1]

polygon_num_ns3={}

for pti in pSNA_tif_n3:
    ...
    if len(da) >= 1:
        polygon_num_ns3[polygon_ID] = len(da[da<=np.percentile(da,num_pencent)])

        
    if len(da)==0:
        continue
    
    im_data[im_data>polygon_num_ns3[polygon_ID]]=np.min(im_data[0:2,])
...
###
pSNA_tif_n5=[i for i in pSNA_tif_Fn if factor in i and "NS5" in i ]
#pSNA_tif_n5=pSNA_tif_n5[0:1]

polygon_num_ns5={}

for pti in pSNA_tif_n5:
    ...
    if len(da) >= 1:
        polygon_num_ns5[polygon_ID] = len(da[da<=np.percentile(da,num_pencent)])

        
    if len(da)==0:
        continue
    
    im_data[im_data>polygon_num_ns5[polygon_ID]]=np.min(im_data[0:2,])
...
pRAN_tif_n3=[i for i in pRAN_tif_Fn if factor in i and "NS3" in i ]
#pRAN_tif_n3=pRAN_tif_n3[0:1]

for prti in pRAN_tif_n3:
    
    polygon_ID=prti.split("_")[3]+"_"+prti.split("_")[4]
    
    if polygon_ID in polygon_num_ns3.keys():
        ...
        pranXY_select=random.sample(pranXY_all,polygon_num_ns3[polygon_ID])
...
pRAN_tif_n5=[i for i in pRAN_tif_Fn if factor in i and "NS5" in i ]
#pRAN_tif_n5=pRAN_tif_n5[0:1]

for prti in pRAN_tif_n5:
    
    polygon_ID=prti.split("_")[3]+"_"+prti.split("_")[4]
    
    if polygon_ID in polygon_num_ns5.keys():
        ...
        pranXY_all=[[i,j] for i in range(im_data.shape[0]) for j in range(im_data.shape[1]) 
                if im_data[i,j]>=0]
    
        pranXY_select=random.sample(pranXY_all,polygon_num_ns5[polygon_ID])
